...
def update_model():
    ...
    try:
        ...
        with model_lock:
            all_data = get_real_user_data()
            train_model(all_data)  # This saves model and place_map
            model, place_map = load_model_and_map()  # Reload updated model

        return jsonify({"message": "✅ Model retrained and updated successfully!"}), 200

    except Exception as e:
        logging.error("❌ Error during model update", exc_info=True)
        return jsonify({"error": str(e)}), 500
